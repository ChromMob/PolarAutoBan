package me.chrommob.polarautoban.webhook;
...
public class Sender {
    ...
    public Sender(ConfigManager configManager) {
        this.config = configManager.getConfigWrapper("config");
    }
    ...
    public void load() {
        ...
        if (enabled) {
            ...
            taskID = Bukkit.getScheduler().scheduleSyncRepeatingTask(PolarAutoBan.getProvidingPlugin(PolarAutoBan.class), this::sendMessages, 20, 20);
        } else {
            Bukkit.getScheduler().cancelTask(taskID);
        }
    }
...
record WebHookMessage(String content, String username) {
